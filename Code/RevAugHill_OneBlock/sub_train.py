...
EPOCHS = 2000
LR = 0.005
MODEL_NAME = "sub_model.pth"

def build_sub_onehot(mod):
...
SUB_ONEHOT = build_sub_onehot(ALPHABET_SIZE)

def exact_sub_target(a_dist, b_dist):
    return torch.einsum("ba,bc,acv->bv", a_dist, b_dist, SUB_ONEHOT)
...
def build_dataset():
    ...
    Y_hard = exact_sub_target(a_hard, b_hard)
    ...
    Y_soft = exact_sub_target(a_soft, b_soft)
